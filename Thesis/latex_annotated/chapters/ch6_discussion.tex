...
The weighted MAE metric assigns weight proportional to test-set size\footnote{\textbf{[Defesa] P:} O MAE é a metric que a mediana minimiza por construção --- deck-stacking? E sob RMSE? \textbf{R:} A mediana ser MAE-optimal é precisamente o que torna batê-la o claim certo (há exploitable signal). Ter RMSE à mão; a direção do resultado é improvável de inverter.}. In an expanding-window design, early scenarios have the largest test sets. Scenario 1 for RDD covers 18 test years; Scenario 14 covers 5. The early scenarios are precisely those where era confound is strongest (post-1990 share 20--28\%) and CarenR underperforms. The scenarios where CarenR wins (10--14) have the smallest test sets among the reliable scenarios and contribute the least to the aggregate metric.
...
The analysis throughout this thesis points to a single underlying limitation: the detrending step uses time as its sole covariate, which conflates climate-driven variation with structural industry changes. The most promising methodological extension is to incorporate structural covariates\footnote{\textbf{[Defesa] P:} Nomeias a correção mas não a fizeste; e os structural covariates correlacionam-se com o tempo --- não recriam o confound? \textbf{R:} Decisão de scope (esforço archival, 1934--2022). A chave é pô-los no detrending model (Decision 1) e testar a stationarity do resíduo antes da rule induction --- como em cointegração.}, vineyard planted area, number of certified producers, annual Port benefício quota, and proportion of espalier vs. pergola training, directly into the detrending model.
...
Once linear detrending was adopted, a natural follow-up question was whether more flexible detrending (LOESS) might produce better results by more accurately capturing the non-linear structural trajectories, particularly the RVV plateau-then-collapse shape. LOESS detrending was evaluated with a smoothing span of 0.40\footnote{\textbf{[Defesa] P:} Porquê um LOESS span de 0,40? \textbf{R:} Dá flexibilidade substancial preservando alguma inter-annual structure. É um stress test (não o production detrend): mostra que melhorar a Decision 1 (RVV 140 para 90 mhl) elimina as rules (Decision 2). Se fosse o detrend principal, o teste seria circular.}, which provides substantial flexibility while preserving some year-to-year structure.
