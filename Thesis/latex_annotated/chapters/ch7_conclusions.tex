...
\noindent •  Exceptional Model Mining extension\footnote{\textbf{[Defesa] P:} Elogias o EMM --- porque não o usaste? \textbf{R:} O EMM ajusta um model por subgroup; com cerca de 12--25 observações por subgroup não há capacidade. É a generalização com princípios (detetar onde a climate-production relationship é excecional) e fica como próximo passo.}: the EMM framework \citep{Duivesteijn2016} generalises distributional subgroup discovery to model-based interestingness criteria. Applying EMM to identify subgroups where the climate-production regression relationship is exceptional, rather than where the marginal distribution shifts, could detect interaction effects that KS-based rules miss.
