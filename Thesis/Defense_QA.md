...
CarenR applies the KS test rule-by-rule, but the search is not an independent 59-test multiple comparison problem. CarenR builds conjunctive rules using a beam search; it does not test all 59 features independently. The significance threshold (p<0.05) is applied to the *final induced rule* as a post-induction filter, not to each candidate feature independently. Additionally, the support threshold (minimum 20% coverage) substantially reduces the effective search space. Nonetheless, I acknowledge that multiple rules from the same dataset are not independent, and report all rules with their raw p-values rather than adjusted ones. A Bonferroni-style correction would be conservative given the beam search structure, but applying FDR correction to the rule set is a valid future methodological extension.
